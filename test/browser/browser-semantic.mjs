...
async function main() {
  ...
  ctrl.abort(reason);
  const scopeErr = await scopeP.then(
    () => null,
    (e) => e,
  );
  ...
  const retryP = retry(
    ...
    { attempts: 5, signal: retryCtrl.signal },
  ).then(
    () => null,
    (e) => e,
  );
  ...
  check(
    'retry:parent-abort',
    retryErr?.message === 'retry-stop' && retryCalls === 1,
  );
  ...
  console.log(
    'BROWSER-SEMANTIC-RESULTS ' +
      JSON.stringify({ results, failedCount: failed.length }),
  );
